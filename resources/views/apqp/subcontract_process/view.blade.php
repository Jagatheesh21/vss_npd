  ...
  @endif
    <div class="card">
        <div class="card-header text-center">
            <b>SubContract Process</b>
        </div>
        <div class="card-body">
            <div class="col-md-12">
                <form id="quote_save" method="POST" enctype="multipart/form-data" action="{{route('subcontract_process.store')}}" >
                  @csrf
                  @method('POST')
                    <div class="row mb-3">
                        ...
                        <div class="col-md-3">
                            <label for="" class="col-sm-8 col-form-label required">File*</label>
                                <a href="{{url($location)}}/{{$subcontract_process_data[0]->file}}" class="form-control btn btn-success btn-sm text-white" target="_blank">Download</a>
                            @error('file')
                            <span class="text-danger">{{$message}}</span>
                            @enderror
                        </div>
                        <div class="col-md-6">
                            ...
                            <textarea name="remarks" class="form-control bg-light" id="remarks" cols="30" rows="5" readonly required>{{$subcontract_process_data[0]->remarks}}</textarea>
                        </div>
                    </div>
                  </form>
            </div>
